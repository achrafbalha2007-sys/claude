% =====================================================================
%  PRESENTATION TIPE (Beamer) - Filiere MP/MPI
%  Modele SIMPLIFIE poutre 1D. Sans MCOT. Beaucoup de schemas TikZ.
%
...
\subtitle{Modèle simplifié « poutre » --- niveau MP/MPI}
\author{TIPE}
...
% ===================== Slide 2 : le constat =====================
\begin{frame}{Le problème : ça casse, et toujours au même endroit}
  \begin{columns}[T]
...
\begin{frame}{Les trois profils que l'on compare}
  ...
  \begin{tikzpicture}[scale=0.95]
    % uniforme
    \begin{scope}
      \draw[verre,thick,fill=verre!12](0,0) rectangle (3,0.45);
      \node at (1.5,1.0){\footnotesize \textbf{Uniforme}};
      \node at (1.5,-0.4){\scriptsize $e(s)=e_0$};
    \end{scope}
    % parabolique (epais aux bords)
    \begin{scope}[xshift=4cm]
      \fill[verre!12] (0,0)--(3,0)--(3,0.75)
        .. controls (1.5,0.15) .. (0,0.75) -- cycle;
      \draw[verre,thick](0,0.75) .. controls (1.5,0.15) .. (3,0.75);
      \draw[verre,thick](0,0)--(3,0)--(3,0.75) (0,0)--(0,0.75);
      \node at (1.5,1.25){\footnotesize \textbf{Parabolique}};
      \node at (1.5,-0.4){\scriptsize $e(s)=e_c+(e_b-e_c)\,s^2$};
    \end{scope}
    % exponentiel
    \begin{scope}[xshift=8cm]
      \fill[verre!12] (0,0)--(3,0)--(3,0.9)
        .. controls (2.2,0.2) and (1.0,0.12) .. (0,0.28) -- cycle;
      \draw[verre,thick](0,0.28) .. controls (1.0,0.12) and (2.2,0.2) .. (3,0.9);
      \draw[verre,thick](0,0)--(3,0)--(3,0.9) (0,0)--(0,0.28);
      \node at (1.5,1.25){\footnotesize \textbf{Exponentiel}};
      \node at (1.5,-0.4){\scriptsize $e(s)=e_c\,e^{\alpha s}$};
    \end{scope}
  \end{tikzpicture}
  \end{center}
  \vspace{0.15cm}
  ...
  \column{0.30\textwidth}
    ...
    \begin{tikzpicture}[scale=0.42]
      % bande de bord (surepaisseur) entre contour exterieur et interieur
      ...
      \node[font=\tiny] at (1.5,6.5){$s=1$ sur tout le bord};
      \node[font=\tiny] at (1.5,-0.6){vue de dessus};
    \end{tikzpicture}
  \column{0.70\textwidth}
    \footnotesize
    \textbf{Attention :} le téléphone est un rectangle, pas un disque. On ne
    prend donc pas la distance au centre mais la \textbf{proximité du bord} :
    \[ s=\max\!\left(\frac{|x|}{L/2},\ \frac{|y|}{\ell/2}\right)\in[0,1] \]
    $s=0$ au centre, $s=1$ sur \textbf{tout} le pourtour $\Rightarrow$ la
    surépaisseur est \textbf{uniforme} le long des quatre bords.\\[2pt]
...
% ===================== Slide 15 : principe Monte-Carlo =====================
\begin{frame}{Étape « simulation » --- la méthode de Monte-Carlo}
  \begin{center}
  \begin{tikzpicture}[node distance=0.55cm and 0.8cm,
      box/.style={draw,rounded corners,fill=verre!10,align=center,
                  font=\scriptsize,text width=2.3cm,minimum height=1cm},
      ar/.style={-{Latex},thick,verre}]
    \node[box](t){\textbf{Tirer} au hasard\\ une chute\\ (hauteur, angle)};
    \node[box,right=of t](c){\textbf{Calculer}\\ $\sigma_{\max}$\\ (modèle poutre)};
    \node[box,right=of c](r){\textbf{Comparer}\\ au seuil $\sigma_c$\\ casse ? oui/non};
    \node[box,right=of r](n){\textbf{Répéter}\\ $N=5000$ fois};
    \node[box,right=of n,fill=ok!15](p){$P=\dfrac{\text{casses}}{N}$};
    \draw[ar](t)--(c);\draw[ar](c)--(r);\draw[ar](r)--(n);\draw[ar](n)--(p);
    \draw[ar,gristik](n.south)+(0,-0.1) .. controls (0,-1.2) and (-6,-1.2) .. (t.south);
  \end{tikzpicture}
  \end{center}
  \vspace{0.3cm}
  \centering\footnotesize On simule des \textbf{milliers de chutes} et on compte
  la proportion qui casse : c'est l'estimation de $P(\text{rupture})$.
\end{frame}

% ===================== Slide 16 : angles realistes =====================
\begin{frame}{On tire des angles \emph{réalistes}, pas au hasard uniforme}
  \begin{columns}[c]
  \column{0.5\textwidth}
    \footnotesize
    \begin{itemize}
      \item Un téléphone tombe rarement parfaitement à plat.
      \item On tire l'angle d'impact selon une \textbf{gaussienne}
            centrée à $\mathbf{65^\circ}$ (souvent sur la tranche / le coin).
      \item Ce sont justement les chutes les plus dangereuses.
    \end{itemize}
    \vspace{0.2cm}
    \centering
    \begin{tikzpicture}[scale=0.7]
      \phone{0}{0}{verre}\node at (0,-1){\scriptsize $\approx 0^\circ$ (plat)};
      \begin{scope}[xshift=2.5cm,rotate=45]\phone{0}{0}{verre}\end{scope}
      \node at (2.5,-1){\scriptsize $\approx 45^\circ$};
      \begin{scope}[xshift=5cm,rotate=80]\phone{0}{0}{danger}\end{scope}
      \node[danger] at (5,-1){\scriptsize $\approx 80^\circ$ (coin)};
    \end{tikzpicture}
  \column{0.5\textwidth}
    \centering
    \begin{tikzpicture}[scale=1]
      \draw[-{Latex}](0,0)--(4.6,0) node[right]{\footnotesize angle ($^\circ$)};
      \draw[-{Latex}](0,0)--(0,2.8);
      \draw[verre,very thick,domain=0.2:4.4,samples=80,smooth]
        plot (\x,{2.3*exp(-(\x-2.9)*(\x-2.9)/0.7)});
      \draw[danger,dashed](2.9,0)--(2.9,2.3);
      \node[danger] at (2.9,-0.35){\footnotesize $65^\circ$};
    \end{tikzpicture}
  \end{columns}
\end{frame}
...
% ===================== Slide 19 : validation RLC =====================
\begin{frame}{Validation 1 --- l'analogie avec un circuit électrique}
  \begin{columns}[c]
  \column{0.46\textwidth}
    \centering
    \begin{tikzpicture}[scale=1]
      ...
      % bobine
      \draw[verre,thick,decorate,decoration={coil,segment length=4pt,amplitude=3pt}](1,2)--(2.2,2);
      \node[verre] at (1.6,2.4){\scriptsize $L\leftrightarrow m$};
      % resistance
      \draw[thick](2.2,2)--(2.5,2);
      \draw[danger,thick](2.5,1.85) rectangle (3.5,2.15);
      \node[danger] at (3.0,2.45){\scriptsize $R\leftrightarrow c$};
      \draw[thick](3.5,2)--(4.5,2)--(4.5,0);
      % condensateur
      \draw[thick](0,0)--(2,0); \draw[ok,thick](2,-0.3)--(2,0.3);
      \draw[ok,thick](2.3,-0.3)--(2.3,0.3); \draw[thick](2.3,0)--(4.5,0);
      \node[ok] at (2.15,-0.6){\scriptsize $C\leftrightarrow 1/k$};
    \end{tikzpicture}
  \column{0.54\textwidth}
    \footnotesize Le ressort amorti et un circuit \textbf{RLC} obéissent à la
    \emph{même équation}. On peut donc \textbf{mesurer} le comportement à
    l'oscilloscope.
    ...
    \end{tabular}
    \end{center}
    On vérifie : période $\propto\sqrt{C}$, image de $T\propto e^{-3/2}$.
  \end{columns}
\end{frame}

% ===================== Slide 20 : experience acoustique =====================
\begin{frame}{Validation 2 --- écouter la rigidité de l'écran}
  \begin{columns}[c]
  \column{0.5\textwidth}
    \centering
    \begin{tikzpicture}[scale=0.9]
      \draw[verre,thick,fill=verre!8,rounded corners=3pt](0,0) rectangle (3,4.2);
      % grille 3x3 de points d'impact
      \foreach \x in {0.6,1.5,2.4}{\foreach \y in {0.7,2.1,3.5}{
        \fill[danger](\x,\y) circle (1.5pt);}}
      % onde sonore vers micro
      \draw[verre](2.4,3.5) ++(0.2,0.2) arc (-30:30:0.4);
      \draw[verre](2.4,3.5) ++(0.35,0.35) arc (-30:30:0.7);
      \node at (1.5,-0.4){\scriptsize grille de 3$\times$3 impacts légers};
    \end{tikzpicture}
  \column{0.5\textwidth}
    \footnotesize On tapote l'écran en plusieurs points et on enregistre le son
    avec un smartphone, puis on fait une \textbf{FFT} :
    \begin{itemize}
      \item zone \textbf{rigide} (épaisse) $\rightarrow$ son \textbf{aigu}
            (haute fréquence)
      \item zone \textbf{souple} $\rightarrow$ son \textbf{grave}
    \end{itemize}
    \vspace{0.15cm}
    \centering
    \begin{tikzpicture}[scale=0.8]
      \draw[-{Latex}](0,0)--(3.6,0) node[right]{\scriptsize $f$};
      \draw[-{Latex}](0,0)--(0,1.8);
      \draw[verre,thick](0.6,0)--(0.6,1.4) node[above]{\scriptsize grave};
      \draw[danger,thick](2.6,0)--(2.6,1.0) node[above]{\scriptsize aigu};
    \end{tikzpicture}\\
    \scriptsize $\Rightarrow$ on relie « basse fréquence » à « forte contrainte ».
  \end{columns}
\end{frame}

% ===================== Slide 21 : limite 1D vs 2D =====================
